...
abstract class $PaginatedResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({int pageNumber, int pageSize, int totalRecords, List<T> data});
}
...
class _$PaginatedResponseImpl<T> implements _PaginatedResponse<T> {
  ...
  @override
  String toString() {
    return 'PaginatedResponse<$T>(pageNumber: $pageNumber, pageSize: $pageSize, totalRecords: $totalRecords, data: $data)';
  }
...
abstract class _PaginatedResponse<T> implements PaginatedResponse<T> {
  const factory _PaginatedResponse(
      ...
      required final List<T> data}) = _$PaginatedResponseImpl<T>;
